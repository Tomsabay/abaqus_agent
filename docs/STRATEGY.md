...
- Implemented: `custom_inp` no-CAE path, orchestrator-produced capsules, capsule manifest helpers, ODB Lens recipe normalization and KPI Markdown reports, physics contract evaluator, Simulation Diff reports, 30+ Solver Doctor patterns, CLI/API/UI report workflows, MCP QA tools, report templates, validation matrix, environment preflight checks, Case Memory search, Case Memory UI, Case Memory artifact/sort/min-score controls, Markdown/HTML report export actions, browser preview/print mode, and report bundle export.
