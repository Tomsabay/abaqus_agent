...
a.Instance(name='Part-1-1', part=mdb.models['{model_name}'].parts['Part-1'], dependent=ON)"""
    ...
    script = f"""# AUTO-GENERATED by abaqus-agent build_model.py
...
p.Set(name='FIXED_END', faces=p.faces.getByBoundingBox(xMin=-0.01, xMax=0.01,
    yMin=-0.01, yMax={W}+0.01, zMin=-0.01, zMax={H}+0.01))
p.Set(name='LOAD_END',  faces=p.faces.getByBoundingBox(xMin={L}-0.01, xMax={L}+0.01,
    yMin=-0.01, yMax={W}+0.01, zMin=-0.01, zMax={H}+0.01))
p.Set(name='TIP_NODES', nodes=p.nodes.getByBoundingBox(xMin={L}-0.01, xMax={L}+0.01,
    yMin={W}/2-{seed}, yMax={W}/2+{seed},
    zMin={H}/2-{seed}, zMax={H}/2+{seed}))
...
mdb.models['{model_name}'].FrequencyStep(
    ...
    minEigen=0.0, maxEigen=None, vectors=None,
    maxIterations=30, blockSize=DEFAULT, maxBlocks=DEFAULT,
    normalization=DISPLACEMENT, propertyEvaluationFrequency=None)
