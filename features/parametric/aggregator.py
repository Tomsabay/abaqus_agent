...
from features.parametric.sweep_engine import SUCCESS_STATUSES
...
def compute_sensitivity(sweep_results: dict) -> dict:
    ...
    # A resumed sweep reports SKIPPED_CACHED; those KPIs came from a real
    # solve too, so they must stay in the sensitivity math.
    completed = [r for r in results if r and r.get("status") in SUCCESS_STATUSES]
...
def save_report(sweep_results: dict, workdir: str | Path) -> Path:
    """Save the sensitivity report next to the sweep it describes.

    The sweep engine resolves its own working directory (env / spec override),
    so the directory recorded in the result wins over the caller's guess —
    otherwise a report lands in a directory that holds no sweep.
    """
    sweep_dir = sweep_results.get("sweep_dir")
    report = generate_sensitivity_report(sweep_results)
    if sweep_dir:
        report_path = Path(sweep_dir) / "sensitivity_report.md"
    else:
        report_path = Path(workdir) / "parametric_sweep" / "sensitivity_report.md"
